...
import {
  ...
  Eye, EyeOff, KeyRound, User, Building2, Shield, AlertCircle,
} from "lucide-react";
...
const MOCK_INVITE = {
  inviterName: "Faiz Ramdhani",
  orgName:     "InsightSphere — Toko Pusat",
  role:        "cashier" as const,
  roleLabel:   "Kasir / Staff",
  email:       "staff.baru@domain.com",
  branch:      "Cabang Selatan",
  expiresAt:   "25 Apr 2026",
};
...
const ROLE_BADGE: Record<string, string> = {
  ...
  admin:             "bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 border-rose-100 dark:border-rose-800/50",
...
export default function AcceptInvitePage() {
  ...
  const roleBadge = ROLE_BADGE[invite.role] ?? ROLE_BADGE.cashier;
  const loginHref = `/login/${invite.role}`;
  ...
  const handleSetup = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!name.trim())                  { setError(t("auth.invite.error_name"));     return; }
    if (password.length < 8)           { setError(t("auth.invite.error_min"));      return; }
    if (password !== confirmPassword)  { setError(t("auth.invite.error_mismatch")); return; }
    setIsLoading(true);
  ...
  const pwFields = [
    { id: "inv-pw",   label: t("auth.invite.password_label"),    val: password,         set: setPassword,         show: showPass,    toggle: () => setShowPass(v => !v) },
    { id: "inv-conf", label: t("auth.invite.confirm_pw_label"),  val: confirmPassword,  set: setConfirmPassword,  show: showConfirm, toggle: () => setShowConfirm(v => !v) },
  ];
          ...
          {step === "invite" && (
            <div className="animate-in fade-in duration-300 space-y-6">
              <div className="text-center space-y-3">
                ...
                <div>
                  ...
                  <p className={cn(T.caption, "text-slate-400 dark:text-slate-500 mt-1")}>
                    {t("auth.invite.invited_by", { name: invite.inviterName })}
                  </p>
                </div>
              </div>
              ...
              <div className={cn(R.lg, "bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-700/60")}>
                {[
                  { icon: Building2, label: t("auth.invite.org"),     value: invite.orgName, isText: true },
                  { icon: Building2, label: t("auth.invite.branch"),  value: invite.branch,  isText: true },
                ].map(row => (
                  <div key={row.label} className="px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <row.icon className="size-3.5 text-slate-400" />
                      <span className={cn(T.label, "text-slate-500 dark:text-slate-400")}>{row.label}</span>
                    </div>
                    <span className={cn(T.bodySm, "font-bold text-slate-800 dark:text-slate-200")}>{row.value}</span>
                  </div>
                ))}
                <div className="px-4 py-3 flex items-center justify-between">
                  ...
                  <span className={cn(T.micro, R.full, "px-2 py-0.5 border", roleBadge)}>
                    {invite.roleLabel}
                  </span>
                </div>
                <div className="px-4 py-3 flex items-center justify-between">
                  <span className={cn(T.label, "text-slate-400 dark:text-slate-500")}>{t("auth.invite.expires")}</span>
                  <span className={cn(T.dataSm, "font-bold text-amber-600 dark:text-amber-400")}>{invite.expiresAt}</span>
                </div>
              </div>
              ...
              <p className={cn(T.caption, "text-center text-slate-400 dark:text-slate-500")}>
                {t("auth.invite.token_label")}: <span className={cn(T.code, "text-slate-500 dark:text-slate-400")}>{String(params.token).slice(0, 16)}…</span>
              </p>
            </div>
          )}
          ...
          {step === "success" && (
            <div className="animate-in fade-in zoom-in-95 duration-300 text-center space-y-5 py-4">
              ...
              <div>
                ...
                <p className={cn(T.bodyEmphasis, "text-indigo-600 dark:text-indigo-400 mt-2")}>{name}</p>
                <p className={cn(T.caption, "text-slate-500 dark:text-slate-400 mt-0.5")}>{invite.roleLabel} · {invite.branch}</p>
              </div>
              <Link
                href={loginHref}
