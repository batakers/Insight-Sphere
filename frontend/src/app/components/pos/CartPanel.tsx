...
export function CartPanel({ cart, onCheckout }: CartPanelProps) {
  ...
  return (
    <aside className="relative flex min-h-[460px] w-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 xl:min-h-0 xl:w-96">
      {/* Header */}
      <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={cn(R.md, "size-9 bg-slate-900 flex items-center justify-center text-white")}>
            <ShoppingBag className="size-4" />
          </div>
          <div>
            <h2 className={cn(T.h3, "text-slate-900 dark:text-slate-100")}>{t("pos.cart.title")}</h2>
            <p className={cn(T.caption, "text-slate-400")}>{t("pos.cart.items", { count: summary.itemCount })}</p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button
            ...
            className={cn(
              ...
              isEmpty ? "text-slate-200 cursor-not-allowed" : "text-amber-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20"
            )}
          >
          ...
          <button
            onClick={clearCart}
            className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-all cursor-pointer"
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2.5 custom-scrollbar">
        {isEmpty ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-8">
            ...
            <p className={cn(T.caption, "leading-loose text-slate-400")}>
          ...
          items.map((item) => {
            ...
            return (
              <div
                key={item.product_id}
                className={cn(R.lg, E.sm, "bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 p-3 group hover:border-indigo-100 dark:hover:border-indigo-900/50 hover:bg-white dark:hover:bg-slate-800 transition-all animate-in slide-in-from-right duration-300")}
              >
                <div className="flex gap-3">
                  <div className={cn(R.md, "size-12 bg-white dark:bg-slate-700 border border-slate-100 dark:border-slate-600 flex items-center justify-center overflow-hidden flex-shrink-0")}>
                    {item.image_url ? (
                      ...
                      <div className={cn(T.code, "text-slate-200")}>POS</div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start gap-1">
                      <h4 className={cn(T.bodySm, "font-bold text-slate-900 dark:text-slate-100 truncate leading-tight flex-1")}>
                        {item.name}
                        {item.is_service && (
                          <span className={cn(T.micro, R.xs, "ml-1 px-1 py-0.5 bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-300")}>SVC</span>
                        )}
                      </h4>
                      <button
                        onClick={() => deleteItem(item.product_id)}
                        className="text-slate-300 hover:text-rose-500 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                      >
                        <X className="size-3" />
                      </button>
                    </div>
                      ...
                      <div>
                        ...
                        {editPriceId === item.product_id ? (
                          <div className="flex items-center gap-1 animate-in slide-in-from-top-1 duration-150">
                            <span className={cn(T.caption, "text-slate-400")}>Rp</span>
                            <input
                            ...
                            <button
                              ...
                              className={cn(T.buttonSm, "text-emerald-600 hover:underline cursor-pointer")}>OK</button>
                            <button
                              type="button"
                              onClick={() => setEditPriceId(null)}
                              aria-label={t("common.cancel")}
                              className={cn(R.sm, FOCUS.ring, "size-7 inline-flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer")}
                            >
                          ...
                          <p className={cn(T.dataEmphasis, "flex items-center gap-1", C.primary.icon)}>
                            ...
                            <span className="text-slate-300 font-bold">/{item.unit}</span>
                            {item.custom_price && (
                              <button
                        ...
                        <p className={cn(T.dataSm, "text-slate-500")}>
                          = {formatRupiah(Math.round(lineTotal))}
                          {discPct > 0 && <span className="ml-1 text-rose-500">−{discPct}%</span>}
                          {item.min_qty && (
                            <span className="ml-1.5 text-indigo-400 flex items-center gap-0.5 inline-flex"><Lock className="size-2" />Min {item.min_qty}</span>
                          )}
                        </p>
                      </div>
                      <div className="flex items-center gap-1.5 bg-white dark:bg-slate-700 border border-slate-100 dark:border-slate-600 rounded-xl p-0.5 shadow-sm">
                        <button
                          onClick={() => removeItem(item.product_id)}
                          disabled={!!(item.min_qty && item.quantity <= item.min_qty)}
                          className={cn(
                            cn(R.sm, "size-6 flex items-center justify-center transition-colors"),
                        ...
                        <span className={cn(T.dataEmphasis, "text-slate-900 dark:text-slate-100 min-w-[18px] text-center")}>{item.quantity}</span>
                        <button
                          onClick={() => addItem(cartItemToProduct(item))}
                          className={cn(R.sm, "size-6 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors cursor-pointer")}
                        >
                  ...
                  {activeDiscountId === item.product_id ? (
                    <div className="flex items-center gap-1.5 animate-in slide-in-from-top-1 duration-150">
                      <input
                        ...
                        autoFocus
                        placeholder="0"
                        className={cn(INPUT.base, INPUT.size.sm, T.label, "w-14 h-7 px-1.5 py-0.5 text-center text-indigo-700 dark:text-indigo-400 border-indigo-300 dark:border-indigo-700")}
                      />
                    ...
                    <button
                      ...
                      className={cn(
                        cn(T.buttonSm, R.xs, "flex items-center gap-0.5 px-1.5 py-0.5 transition-all cursor-pointer"),
                        discPct > 0
      ...
      {/* Summary Footer */}
      <div className="p-5 bg-slate-900 rounded-t-[32px] shadow-2xl shadow-slate-900/50 border-t border-slate-800">
        <div className="space-y-2.5 mb-5">
          <div className="flex justify-between items-center text-slate-400">
            <span className={cn(T.caption, "text-slate-400")}>{t("pos.cart.subtotal")}</span>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-1.5">
              ...
              <span className={cn(T.caption, "text-slate-500")}>{t("pos.cart.discount")}</span>
              <input
                type="number" min="0" max="100"
                value={txnDiscountPct || ""}
                onChange={e => setTxnDiscount(parseInt(e.target.value) || 0)}
                placeholder="0"
                className={cn(T.dataSm, R.sm, "w-10 bg-slate-800 border border-slate-700 text-white placeholder:text-slate-500 text-center px-1 py-0.5", FOCUS.ring)}
              />
              <span className={cn(T.caption, "text-slate-600")}>%</span>
            </div>
            {summary.discount > 0 && (
          ...
          <div className="flex justify-between items-center pt-1">
            <span className={cn(T.h3, "text-white")}>TOTAL</span>
        ...
        <button
          ...
          onClick={onCheckout}
          className={cn(
            "w-full py-3.5 rounded-2xl flex items-center justify-center gap-3 transition-all cursor-pointer shadow-xl", T.buttonSm,
            isEmpty
        ...
        <div className={cn(T.caption, "flex items-center justify-center gap-4 mt-4 text-slate-600")}>
