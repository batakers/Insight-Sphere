...
export function ProductCard({ product, onAdd }: ProductCardProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "group relative bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-3 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-100/50 dark:hover:shadow-indigo-900/20 hover:-translate-y-1 cursor-pointer flex flex-col text-left w-full",
    ...
    >
      ...
      {/* Image / Icon */}
      <div className="relative aspect-[4/3] bg-slate-50 dark:bg-slate-800 rounded-xl overflow-hidden mb-3 border border-slate-50 dark:border-slate-700 ring-4 ring-white dark:ring-slate-900 shadow-inner flex items-center justify-center group-hover:bg-indigo-50/50 dark:group-hover:bg-indigo-900/20 transition-colors">
        {product.image_url ? (
        ...
        {!isOutOfStock && (
          <div className="absolute inset-0 bg-indigo-600/0 group-hover:bg-indigo-600/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
            <div className={cn(R.full, "bg-indigo-600 text-white size-12 flex items-center justify-center shadow-xl shadow-indigo-200 dark:shadow-indigo-900/50 scale-50 group-hover:scale-100 transition-all duration-300")}>
              {product.custom_price ? <Pencil className="size-5" /> : <Plus className="size-6 stroke-[3]" />}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 space-y-1 mt-1">
        ...
        <p className={cn(T.caption, "text-slate-400 truncate")}>{product.category}</p>
        <span className={cn(T.code, "text-slate-300")}>{product.sku}</span>
      </div>

      {/* Price footer */}
      <div className="mt-3 pt-2 border-t border-slate-50 dark:border-slate-800 flex flex-col gap-0.5">
        {product.custom_price ? (
          ...
          <span className={cn(T.body, "font-bold text-indigo-600 leading-tight")}>
            ...
            <span className={cn(T.caption, "text-slate-400 ml-0.5")}>/{product.unit}</span>
          </span>
        )}
        {!product.is_service && (
