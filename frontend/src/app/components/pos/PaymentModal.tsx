...
export function PaymentModal({ cart, checkout, onClose }: PaymentModalProps) {
  ...
  const printReceipt = () => {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Struk InsightSphere</title>
      <style>
        ...
        th { border-bottom: 1px solid #000; padding: 4px 8px; font-size: 11px; }
        .total-row td { font-weight: bold; border-top: 1px dashed #000; padding-top: 6px; }
        .footer { text-align: center; margin-top: 12px; font-size: 11px; }
      </style></head><body>
      <h2>InsightSphere POS</h2>
      <p class="center" style="font-size:11px;margin:2px 0">${dateStr} &mdash; ${timeStr}</p>
      <p class="center" style="font-size:10px;margin:2px 0">ID: ${lastTransaction?.id ?? "-"}</p>
      ...
      <table style="width:100%;border-collapse:collapse">
        ...
        <tfoot>
          <tr class="total-row"><td colspan="3">TOTAL</td><td style="text-align:right">${formatRupiah(summary.total)}</td></tr>
          ${method === "CASH" ? `<tr><td colspan="3">Tunai</td><td style="text-align:right">${formatRupiah(cashAmount)}</td></tr><tr><td colspan="3">Kembalian</td><td style="text-align:right">${formatRupiah(cashAmount - summary.total)}</td></tr>` : `<tr><td colspan="3">QRIS</td><td style="text-align:right">Lunas</td></tr>`}
        </tfoot>
      </table>
      <div class="footer"><div class="divider"></div><p>Terima kasih atas kunjungan Anda!</p><p>InsightSphere &mdash; Smart Retail POS</p></div>
      </body></html>`;
  ...
  if (isSuccess) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl animate-in fade-in duration-300">
        <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[48px] p-10 text-center shadow-2xl border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 slide-in-from-bottom-4 duration-200">
           ...
           <div className="flex gap-4">
              <button 
                onClick={onClose}
                className={cn(T.buttonLg, R.lg, "flex-1 py-4 px-6 bg-slate-900 text-white hover:bg-slate-800 transition-all cursor-pointer shadow-xl shadow-slate-200")}
              >
                {t("pos.pay.close_f5")}
              </button>
              <button
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 w-full max-w-4xl rounded-[48px] overflow-hidden shadow-2xl flex flex-col md:flex-row border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 slide-in-from-bottom-4 duration-200">
        ...
        <div className="md:w-7/12 p-10 bg-white dark:bg-slate-900 relative">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                 {([
                   ...
                   <button
                     ...
                     className={cn(
                       "relative p-4 rounded-[24px] border-2 transition-all flex flex-col items-center gap-2 cursor-pointer",
           ...
           <div className="space-y-5">
              {method === "CASH" && (
                <div className="animate-in slide-in-from-top-4 duration-300">
                   ...
                   <div className="relative group">
                      <div className="absolute left-6 top-1/2 -translate-y-1/2 text-2xl font-bold text-slate-300 group-focus-within:text-indigo-600 transition-colors">Rp</div>
                      <input
                        type="number"
                        value={cashReceived}
              ...
              {method === "QRIS" && (
                <div className="flex flex-col items-center justify-center p-8 bg-slate-50 rounded-[40px] border border-dashed border-slate-200 animate-in slide-in-from-top-4 duration-300">
                   <div className="w-48 h-48 bg-white dark:bg-slate-800 p-4 rounded-3xl shadow-sm mb-4 border border-slate-100 dark:border-slate-700">
                      <div className="w-full h-full bg-slate-100 flex items-center justify-center relative group">
                        ...
                        <div className={cn("absolute inset-x-0 bottom-0 bg-indigo-600 text-white uppercase py-1.5 text-center translate-y-full group-hover:translate-y-0 transition-transform", T.micro)}>Auto-Generated</div>
                      </div>
                   </div>
                   ...
                   {splitCashAmt > 0 && splitCashAmt < summary.total && (
                     <div className="grid grid-cols-2 gap-3 p-4 bg-slate-900 rounded-2xl animate-in slide-in-from-top-2 duration-200">
                       <div className="text-center">
                         <p className={cn(T.label, "text-slate-500 mb-1")}>Tunai</p>
                         <p className={cn(T.h2, "text-white font-data")}>{formatRupiah(splitCashAmt)}</p>
                       </div>
                       <div className="text-center">
                         <p className={cn(T.label, "text-slate-500 mb-1")}>QRIS</p>
           ...
           <div className="mt-10">
              <button
                ...
                className={cn(
                  "w-full py-6 rounded-[32px] flex items-center justify-center gap-4 transition-all cursor-pointer shadow-2xl", T.buttonLg,
                  isSubmitting || (method === "CASH" && cashAmount < summary.total) || (method === "SPLIT" && (splitCashAmt <= 0 || splitCashAmt >= summary.total))
                    ? "bg-slate-100 text-slate-400 border border-slate-200 cursor-not-allowed"
                    : "bg-indigo-600 text-white hover:bg-slate-900 shadow-indigo-200 active:scale-95"
                )}
              >
                {isSubmitting ? (
