...
const MOCK_HISTORY: StockHistoryEntry[] = [
...
export function StockHistoryTable({ onClose }: StockHistoryTableProps) {
  ...
  const paginatedData = MOCK_HISTORY.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className={cn(R.xl, E["2xl"], "bg-white dark:bg-slate-900 w-full max-w-4xl overflow-hidden border border-slate-200/50 dark:border-slate-700 animate-in slide-in-from-bottom-8 duration-500 flex flex-col max-h-[85vh]")}>
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={cn(T.caption, "text-slate-400")}>
                {MOCK_HISTORY.length} entries
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => exportHistoryToCSV(MOCK_HISTORY, `riwayat-stok-${new Date().toISOString().slice(0,10)}.csv`)}
        ...
        >
          <table className={TABLE.base} aria-label={t("inv.history.title")}>
            <thead className={cn(TABLE.head, TABLE.stickyHead, "bg-slate-50 dark:bg-slate-800")}>
              <tr>
                <th className={cn(TABLE.headCell, "sticky left-0 z-20 bg-slate-50 dark:bg-slate-800")}>{t("inv.history.date")}</th>
              ...
              {paginatedData.map(entry => {
                ...
                return (
                  <tr key={entry.id} className={cn(TABLE.row, TABLE.rowHover, "group")}>
                    <td className={cn(TABLE.cell, T.dataSm, "sticky left-0 z-10 bg-white dark:bg-slate-900 group-hover:bg-slate-50 dark:group-hover:bg-slate-800/50 font-bold text-slate-500 dark:text-slate-400 whitespace-nowrap")}>{entry.date}</td>
